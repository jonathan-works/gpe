...
public class ImportarXPDL implements Serializable {
	...
	private void gereFluxo(List<Element> poolsList, List<Element> workflowList,
			String cdFluxo) {
		...
		List<Element> workflows = obterElements(workflowList.get(0),
				"WorkflowProcess");
	...
	private void gereXML(String nome, List raias, List nos, List transicoes,
		...
		try {
			...
			System.err.println("Classe Java: ImportarXPDL. Erro msg: "
					+ e.getMessage());
	...
	@SuppressWarnings(RAWTYPES)
	private void addNodes(String name, List nos, List transicoes,
			ProcessDefinition definition) throws InvalidActivityException,
			ParallelNodeException {
		...
		for (int i = 0; i < nos.size(); i++) {
			...
			if (type == null) {
				throw new InvalidActivityException(
						"O Sistema e-PA não aceita o Nó do tipo: "
								+ ele.getName());
			}
	 ...
	 */
	private void transformeParallelNode(List<Node> lista)
			throws ParallelNodeException {
		if (lista != null) {
			...
			Node temp = null;
			Set<Transition> arrives = null;
			List<Transition> leaves = null;
			while (iterator.hasNext()) {
				temp = iterator.next();
				arrives = temp.getArrivingTransitions();
				leaves = temp.getLeavingTransitions();
				if (arrives == null || arrives.isEmpty() || leaves == null
						|| leaves.isEmpty()
						|| (arrives.size() > 1 && leaves.size() > 1)) {
					String name = temp.getName();
					throw new ParallelNodeException(
							"Impossível verificar se o Nó ("
									+ (name != null ? name : "Paralelo")
					...
					throw new ParallelNodeException(
							"Impossível verificar se o Nó ("
									+ (temp.getName() != null ? temp.getName()
											: "Paralelo")
									+ ") é do tipo Join ou Fork.");
			}
		}
	...
	private void preencherNo(List<Node> lista, Map<Node, Integer> parallel,
			Node temp, Set<Transition> arrives, List<Transition> leaves,
			Node node) {
	...
	public String verificarTypenode(Element ele) {
		...
		if (event != null && !event.isEmpty()) {
			...
			List inter = temp.getChildren("IntermediateEvent",
					temp.getNamespace());
			if (start != null && !start.isEmpty()) {
				...
				String value = temp2.getAttributeValue("Trigger",
						temp2.getNamespace());
			...
			if (gateway != null && !gateway.isEmpty()
					&& PARALLEL.equalsIgnoreCase(gateway))
				tipo = PARALLEL;
			else
				tipo = "Decision";
		}
			...
			if (nodeType != null) {
				...
				if (nome != null && !nome.isEmpty())
					...
					if (node instanceof StartState || node instanceof EndState)
						node.setName((node instanceof StartState ? "Início"
								: "Fim"));
					else
	...
	public void addTask(Node node, ProcessDefinition definition) {
		if (node instanceof TaskNode) {
			...
			t.setSwimlane(definition.getTaskMgmtDefinition().getSwimlanes()
					.values().iterator().next());
			tn.setEndTasks(true);
			tn.addTask(t);
		} else if (node instanceof StartState) {
			Task startTask = new Task("Tarefa inicial");
			startTask.setSwimlane(definition.getTaskMgmtDefinition()
					.getSwimlanes().values().iterator().next());
			TaskHandler startTaskHandler = new TaskHandler(startTask);
			definition.getTaskMgmtDefinition().setStartTask(
					startTaskHandler.getTask());
		} else if (node instanceof ProcessState) {
			ProcessState sub = (ProcessState) node;
			ProcessDefinition subProc = ProcessDefinition
					.createNewProcessDefinition();
	...
	private class ParallelNode extends DecisionNode {
		...
		public boolean equals(Object o) {
			if (o instanceof DecisionNode)
				return this.getName().equals(((DecisionNode) o).getName());
			return false;
		}

...
public class SugiyamaLayoutAlgorithm {
     ...
     * member <code>levels</code>. The Member levels contains List Objects and
     * the List per level contains Cell Wrapper Objects. After that the
     ...
     * 
     */
    public void perform(JGraph jgraph, boolean applyToAll,
        ...
        /*
         * The Algorithm distributes the nodes on a grid. For this grid you can
         * configure the vertical spacing. This field specifies the configured
         * value
         */
        ...
        checkEgdes(jgraph, selectedCellViews);

        // draw the graph in the window
        drawGraph(jgraph, levels, min, spacing, true);
    ...
    private void checkEgdes(JGraph jgraph, CellView[] selectedViews) {
        GraphModel model = jgraph.getModel();
        for (int i = 0; i < selectedViews.length; i++) {

            if (!(selectedViews[i] instanceof VertexView)) {
                continue;
            }

            CellView vertexViewToInspect = selectedViews[i];
            Object vertex = vertexViewToInspect.getCell();

            int portCount = model.getChildCount(vertex);
            for (int j = 0; j < portCount; j++) {
                DefaultPort port = (DefaultPort) model.getChild(vertex, j);
                List<Object> edges = new ArrayList<Object>();
                Iterator itrEdges = model.edges(port);
                while (itrEdges.hasNext() && port.getEdges().size() > 2) {
                    Object edge = itrEdges.next();
                    if (model.getSource(edge) == port) {
                        DefaultEdge de = (DefaultEdge) edge;
                        edges.add(model.getParent(de.getTarget()));
                    }
                }
            }
        }
     ...
     * Cell. The Roots are stored in the List named roots
     * 
     ...
     * 
     * Each level was represended by a List with Cell Wrapper objects. These
     * Lists are the elements in the <code>levels</code> List.
     * 
     */
    ...
    /**
     * Fills the List for the specified level with a wrapper for the
        ...
        for (int i = 0; i < allSelectedViews.length; i++) {
            ...
            for (int k = 0; k < portCount; k++) {
                ...
                while (edges.hasNext()) {
                    ...
                    if (neighborVertexView == vertexView) continue;
                    ...
                    if (currentwrapper == null || neighborWrapper == null
                            || currentwrapper.level == neighborWrapper.level) continue;
            ...
            for (int i = 0; i < portCount; i++) {
                ...
                while (edges.hasNext()) {
                    ...
                    if (targetWrapper == null
                            || targetWrapper.getLevel() == levelIndex) continue;
            ...
            if (countNodes > 0) {
                ...
                while (newGridPosition != sourceWrapper.getGridPosition()
                    ...
                    if (verbose) {
                        ...
                        System.out.print("try move at Level " + levelIndex
                                + " with index " + currentIndexInTheLevel
                                + " to " + (toRight ? "Right" : "Left")
                                + " CurrentGridPos: " + tmpGridPos
    ...
    /**
     * @param toRight <tt>true</tt> = try to move the currentWrapper to right;
     *        <tt>false</tt> = try to move the currentWrapper to left;
     * @param currentLevel List which contains the CellWrappers for the current
     *        level
    ...
    protected void drawGraph(JGraph jgraph, List levels, Point min,
        ...
        for (int rowCellCount = 0; rowCellCount < levels.size(); rowCellCount++) {
            ...
            for (int colCellCount = 0; colCellCount < level.size(); colCellCount++) {
                ...
                Rectangle2D bounds = (Rectangle2D) view.getBounds().clone();

                // adjust
                int x = min.x + spacing.x * wrapper.getGridPosition();
                int y = min.y + spacing.y * rowCellCount;
    ...
    class CellWrapper implements Comparable {
        ...
        public String toString() {
            return this.vertexView.getCell().toString();
        }

	...
	private String makeMap() {
		...
		for (CellView view : graph.getGraphLayoutCache().getCellViews()) {
			if (view.getCell() instanceof JbpmDefaultCell) {
				...
				sb.append("nodes.addToMap('")
					.append(cell.getNodeIndex())
					.append("','")
					.append(r.getMinX() + 5).append(",")
					.append(r.getMinY() + 5).append(",")
					.append(r.getMaxX() + 5).append(",")
					.append(r.getMaxY() + 5).append("', '")
...
//      facade the JGraph instance
            JGraphHierarchicalLayout layout = new JGraphHierarchicalLayout(); // Create an
...
//      the actual graph
	...
	private DefaultGraphCell addNode(Node node, Map<Node, DefaultGraphCell> nodes, List<DefaultGraphCell> cellList) {
	...
	private void addEdge(DefaultGraphCell from, DefaultGraphCell to, List<DefaultGraphCell> cellList) {

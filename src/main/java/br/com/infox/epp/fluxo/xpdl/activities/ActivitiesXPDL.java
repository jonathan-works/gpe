...
import br.com.infox.epp.fluxo.xpdl.transition.TransitionXPDL;
...
public class ActivitiesXPDL implements Serializable {
    ...
    private static List<ActivityXPDL> createAtivitiesList(List<Element> list) throws ActivityNotAllowedXPDLException {
        ...
        for (Element ele : list) {
            activityXPDLList.add(ActivityXPDLFactory.createInstance(ele, FluxoXPDL.NO_NAME
                    + index));
            index++;
        }
    ...
    private void createForkJoinActivityXPDL(List<ActivityXPDL> lista,
            ParallelActivityXPDL parallel, int position,
            List<TransitionXPDL> transitions) throws ParallelNodeXPDLException {
        List<TransitionXPDL> parallelTransitions = getAllTransitionsContainsActivity(transitions, parallel);
        List<ActivityXPDL> arrives = parallel.getArrives();
        List<ActivityXPDL> leaves = parallel.getLeaves();
        if (arrives != null && leaves != null && arrives.size() == 1
                && leaves.size() > 1) {
            createForknode(lista, parallelTransitions, parallel, position);
        } else if (arrives != null && leaves != null && leaves.size() == 1
                && arrives.size() > 1) {
    ...
    private void createJoinNode(List<ActivityXPDL> lista,
            List<TransitionXPDL> parallelTransitions,
            ParallelActivityXPDL parallel, int position) {
    ...
    private Map<ParallelActivityXPDL, Integer> getAllParallelXPDL(

...
public class AnexarDocumentosView implements Serializable {
    ...
    private DocumentoTemporario documentoUploader;
    private byte[] data;
	...
	public void newUploaderInstance() {
		DocumentoTemporario newUploader = new DocumentoTemporario();
		newUploader.setDocumentoBin(new DocumentoBin());
		newUploader.setAnexo(Boolean.TRUE);
		if (pastaDefault != null) {
			newUploader.setPasta(pastaDefault);
		}
		setDocumentoUploader(newUploader);
    ...
    public void fileUploadListener(FileUploadEvent fileUploadEvent) {
        ...
        try {
            data = IOUtils.toByteArray(uploadedFile.getInputStream());
            ClassificacaoDocumento classificacaoDocumento = getDocumentoUploader().getClassificacaoDocumento();
            documentoUploaderService.validaDocumento(uploadedFile, classificacaoDocumento, data);
            getDocumentoUploader().setDocumentoBin(documentoUploaderService.createProcessoDocumentoBin(uploadedFile));
    ...
    public void clearUploadFile() {
    	getDocumentoUploader().setDocumentoBin(new DocumentoBin());
    	setShowUploader(getDocumentoUploader().getClassificacaoDocumento() != null);
    	setShowUploaderButton(false);
    }
    ...
    public void persistUpload() {
        try {
            if (getDocumentoUploader().getDocumentoBin().getExtensao() == null) {
                FacesMessages.instance().add("Não foi anexado nenhum documento.");
                return;
            }
        	getDocumentoUploader().setProcesso(getProcesso());
        	if (getDocumentoUploader().getPasta() == null) {
        		getDocumentoUploader().setPasta(pastaDefault);
        	}
            documentoTemporarioManager.gravarDocumentoTemporario(getDocumentoUploader(), data);
            getDocumentoTemporarioList().add(new DocumentoTemporarioWrapper(getDocumentoUploader()));
            newUploaderInstance();
	...
	public DocumentoTemporario getDocumentoUploader() {
    	if (documentoUploader == null) {
    		newUploaderInstance();
    	}
        return documentoUploader;
    }

    public void setDocumentoUploader(DocumentoTemporario documentoUploader) {
        this.documentoUploader = documentoUploader;
    }

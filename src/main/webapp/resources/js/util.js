...
namespace("infox",{
	...
	applyCSS:function applyCSS(args) {
	    ...
	    var defaultDiacriticsRemovalMap = [{
            ...
            return str;
	}, markHideUnmatched:function (regex, target){
	    if (regex.test(this.sanitize(target.textContent.trim()))) {
	        target.classList.remove('hidden');
            } else {
                target.classList.add('hidden');
            }
	}, markHideUnmatchedObjects:function (query, items){
	    var regex = new RegExp(['.*', this.sanitize(query), '.*'].join(''), 'i');
            for (var i = 0, l = items.length; i < l; i++) {
                this.markHideUnmatched(regex, items.item(i));
            }
	}, filter:function (object){
            this.markHideUnmatchedObjects(object.input.value, document.querySelectorAll(object.filtered));
	}, addFilter(object){
